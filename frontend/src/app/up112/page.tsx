...
export default function Up112Page() {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      const up112Records = stored.filter((r: any) => r.forwardedTarget === '112');
      setComplaints(up112Records);
      setVerifications([]);
      setCertificates([]);
      setEvents([]);
    } catch (e) {
    ...
    <div className="h-screen w-full flex flex-col bg-slate-50 dark:bg-slate-950 overflow-hidden text-slate-600 dark:text-slate-300 font-sans">
      ...
      <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-6 shrink-0 z-30 shadow-md">
        <div className="flex items-center space-x-4">
          ...
          <div>
            <h1 className="text-lg font-bold text-slate-900 dark:text-white tracking-wide">UP-112 Emergency Dispatch Center</h1>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        <aside className="w-[400px] bg-white dark:bg-slate-900 border-l border-slate-200 dark:border-slate-800 flex flex-col shrink-0 z-20 shadow-lg relative p-6">
          <h2 className="text-lg font-bold text-slate-900 dark:text-white mb-4">Live Dispatch Feed</h2>
          <p className="text-sm text-slate-500">Awaiting incoming dispatches...</p>
        </aside>
                ...
                <div className="p-6 border-t border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <button
                      onClick={() => {
                        const stored = JSON.parse(localStorage.getItem('forwarded_records') || '[]');
                        stored.push({ ...selectedRecord, forwardedTarget: 'station', forwardedAt: new Date().toISOString() });
                        localStorage.setItem('forwarded_records', JSON.stringify(stored));
                        alert('Forwarded to Local Police Station!');
                      }}
                      className="py-2 px-3 rounded-lg text-xs font-bold transition-all border bg-police-navy text-white hover:bg-blue-900 col-span-2 mt-2"
                    >
                      Forward to Local Police Station
                    </button>
                  </div>
                </div>
              ) : (
                <div className="p-6 border-t border-police-red/20 bg-police-red/5">
                  <h4 className="text-xs font-bold text-police-red uppercase tracking-widest mb-3">Emergency Response Actions</h4>
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      disabled={updatingId === selectedRecord.referenceNumber || selectedRecord.status === 'RESOLVED'}
                      onClick={async () => {
                        setUpdatingId(selectedRecord.referenceNumber);
                        try {
                          await EmergencyService.resolveAlert(selectedRecord.referenceNumber, 'admin-demo');
                          setSelectedRecord((prev: any) => ({ ...prev, status: 'RESOLVED' }));
                          const stored = JSON.parse(localStorage.getItem('forwarded_records') || '[]');
                          stored.push({ ...selectedRecord, status: 'RESOLVED', forwardedTarget: '112', forwardedAt: new Date().toISOString() });
                          localStorage.setItem('forwarded_records', JSON.stringify(stored));
                          alert('Resolved & Forwarded to UP-112!');
                        } catch (e) {
                      ...
                      className="py-2 px-3 rounded-lg text-xs font-bold transition-all border bg-police-red text-white hover:bg-red-700 disabled:opacity-50"
                    >
                      {selectedRecord.status === 'RESOLVED' ? 'Resolved' : 'Forward to UP-112 (Resolve)'}
                    </button>
                    <button
                      disabled={updatingId === selectedRecord.referenceNumber || selectedRecord.status === 'RESOLVED'}
                      onClick={async () => {
                        ...
                        try {
                          ...
                          setSelectedRecord((prev: any) => ({ ...prev, status: 'RESOLVED' }));
                          const stored = JSON.parse(localStorage.getItem('forwarded_records') || '[]');
                          stored.push({ ...selectedRecord, status: 'RESOLVED', forwardedTarget: 'station', forwardedAt: new Date().toISOString() });
                          localStorage.setItem('forwarded_records', JSON.stringify(stored));
                          alert('Resolved & Forwarded to Local Police Station!');
                        } catch (e) {
                          alert('Failed to resolve emergency.');
                        } finally {
                          setUpdatingId(null);
                      ...
                      className="py-2 px-3 rounded-lg text-xs font-bold transition-all border bg-police-gold text-police-navy-dark hover:bg-yellow-500 disabled:opacity-50"
                    >
                      {selectedRecord.status === 'RESOLVED' ? 'Resolved' : 'Dispatch Local Police (Resolve)'}
                    </button>
                  </div>
                </div>
              )}
